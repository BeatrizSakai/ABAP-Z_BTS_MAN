CLASS zcl_bts_cargas DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_bts_cargas IMPLEMENTATION.

    METHOD if_oo_adt_classrun~main.

    TYPES: tt_product  TYPE TABLE OF ztbts_product WITH DEFAULT KEY,
           tt_vendor   TYPE TABLE OF ztbts_vendor WITH DEFAULT KEY,
           tt_company  TYPE TABLE OF ztbts_company WITH DEFAULT KEY,
           tt_customer TYPE TABLE OF ztbts_customer WITH DEFAULT KEY.
    ...
    DELETE FROM ztbts_product.
    DELETE FROM ztbts_vendor.
    DELETE FROM ztbts_company.
    DELETE FROM ztbts_customer.

    MODIFY ztbts_product FROM TABLE @lt_product.
    MODIFY ztbts_vendor FROM TABLE @lt_vendor.
    MODIFY ztbts_company FROM TABLE @lt_company.
    MODIFY ztbts_customer FROM TABLE @lt_customer.
